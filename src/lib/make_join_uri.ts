...
const makeJoinUri = (roomName: string, pass: any, pid: number, mode: number): string => {
  const urienc = (str: string | number): string => {
    return encodeURIComponent(str).replace(/[!*'()]/g, (c) => {
      return '%' + c.charCodeAt(0).toString(16);
    });
  };

  const str: string = 'joingroup?mode=' + urienc(mode) + '&pid=' + urienc(pid) + '&nickname=&groupname=' + urienc(roomName) + '&password=' + urienc(pass);
